    ...
    mkIf cfg.enable {
      fileSystems = mapAttrs (name: value: {
        ...
        options =
          ...
          ++ optionalNull value.fileCreationPolicy.with-perms "create-with-perms=${value.fileCreationPolicy.with-perms}"
          ++ optionalNull value.force.user "force-user=${value.force.user}"
          ++ optionalNull value.force.group "force-group=${value.force.group}"
          ++ optionalNull value.perms "perms=${value.perms}"
